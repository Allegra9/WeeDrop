class Buyer < ApplicationRecord
  ...
  validates :cardholder_name, presence: true, format: { with: /\b[^\d\W]+\b/i, message: "can only have valid characters" }, length: 2..20
  validates :card_number, presence: true, numericality: { only_integer: true }, length: { is: 16 }

  validates :email_address, format: { with: /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\z/i}, presence: true

  validates :cvv, presence: true, numericality: { only_integer: true }, length: { is: 3 }
  validates :card_expiry_year, presence: true

  validates :billing_address, presence: true, length: 2..20
  validates :delivery_address, presence: true, length: 2..20
